...
from model import HeatTransfer, plot_result, calculate_inner_omega
...
class Theta(HeatTransfer):
    def __init__(self, dim: int, goal: str, initial: str, boundary: str, h_const: float, k_const: float, omega: str, time_range: float, theta: float = 0.3) -> None:
    ...
    def initialize_matrices(self):
        if self.dim == 1:
            ...
            np.fill_diagonal(A_matrix, 1-(2*(1-self.theta))*l_const)
            np.fill_diagonal(B_matrix, 1+(2*self.theta)*l_const)

            # adding offset diagonal values.
            offset_A_matrix = np.array([(1-1*self.theta)*l_const for _ in range(0, A_matrix.shape[0] - 1)])
            offset_B_matrix = np.array([(-1*self.theta)*l_const for _ in range(0, B_matrix.shape[0] - 1)])
            ...
            # filling diagonal
            np.fill_diagonal(differential_A_matrix, 1-(4*(1-self.theta))*l_const)
            np.fill_diagonal(differential_B_matrix, 1+(4*self.theta)*l_const)

            # filling main offset diagonals.
            offset_A_main = np.array([(1-1*self.theta)*l_const for _ in range(0, differential_A_matrix.shape[0] -1)])
            offset_B_main = np.array([(-1*self.theta)*l_const for _ in range(0, differential_B_matrix.shape[0] -1)])
            ...
            offset_A_secondary = np.array([(1-1*self.theta)*l_const for _ in range(0, A_matrix.shape[0] - space.shape[0])])
            offset_B_secondary = np.array([(-1*self.theta)*l_const for _ in range(0, B_matrix.shape[0] - space.shape[0])])
    ...
    def calculate_initial_1d(self, points: list) -> np.ndarray:
        ...
        for point in points:
            ...
            values[point] += (1-1*self.theta)*self.k * eval(self.f, {"x": x, "t": 0, "math": math})
            values[point] += (self.theta)*self.k * eval(self.f, {"x": x, "t": self.k, "math": math})
        return values
    ...
    def calculate_initial_nd(self, points: list) -> np.ndarray:
        ...
        for point in points:
            ...
            values[point] += (1-1*self.theta)*self.k * eval(self.f, {"x": x, "y": y, "t": 0, "math": math})
            values[point] += (self.theta)*self.k * eval(self.f, {"x": x, "y": y, "t": self.k, "math": math})
        return values

    def calculate_initial(self, plot: bool = False) -> np.ndarray:
    ...
    def calculate_solution_1d(self, initial_values: np.ndarray, time_steps: np.ndarray) -> np.ndarray:
        ...
        for t in time_steps:
            ...
            f_values = np.array([(1-1*self.theta)*self.k * eval(self.f, {"x": point, "t": t, "math": math}) for point in points])
            new_values += f_values
            f_values = np.array([(self.theta)*self.k * eval(self.f, {"x": point, "t": t+self.k, "math": math}) for point in points])
            new_values += f_values
    ...
    def calculate_solution_nd(self, initial_values: np.ndarray, time_steps: np.ndarray) -> np.ndarray:
        ...
        for t in time_steps:
            ...
            f_values = np.matrix.transpose(np.array([(1-1*self.theta)*self.k * eval(self.f, {"x": x, "y": y, "t": t, "math": math}) for (x,y) in points]))
            new_values += f_values
            f_values = np.matrix.transpose(np.array([(self.theta)*self.k * eval(self.f, {"x": x, "y": y, "t": t+self.k, "math": math}) for (x,y) in points]))
            new_values += f_values
...
if __name__ == "__main__":
    ...
    # creating the model.
    model = Theta(dim, f_equation, v_equation, g_equation, h, k, omega, time_range, 1)
